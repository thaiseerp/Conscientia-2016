...
class Astronomia extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->ion_auth->logged_in()){
            ...
            if($this->astronomia_user_model->get_user())
            {
                $role = $this->astronomia_user_model->get_role();
                if($role == -1)
                {
                ...
                {   //Load the level
                    $this->load->model('astronomia_levels_model');
                    $level_details = $this->astronomia_levels_model->get_level();
    ...
    public function register()
    {
        ...
        if($this->ion_auth->logged_in())
        {
            ...
            {
                $this->load->model('astronomia_user_model');
                if($this->astronomia_user_model->register())
                {
                    redirect('astronomia', 'refresh');
                }
            }
            else
            {
	...
	public function answer(){
		...
		$this->load->model('astronomia_levels_model');
		$result = $this->astronomia_levels_model->check_answer($answer);
	...
	public function leaderboard(){
		...
		$leaderboard = $this->astronomia_status_model->get_leaderboard();

		$data['page'] = 'leaderboard';
		$data['leaderboard'] = $leaderboard;
		$data['header_data'] = array('title' => 'Conscientia 2016 | Astronomia Leaderboard');
		$this->load->view('astronomia/template',$data);
	}

	public function rules(){
		$data['page'] = 'rules';
		$data['header_data'] = array('title' => 'Conscientia 2016 | Astronomia Rules');
		$this->load->view('astronomia/template',$data);
	}
	...
	public function levels(){
		...
		if (file_exists('./contents/events/astronomia/'.$file) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE']))
		{
		...
		$this->load->model('astronomia_user_model');
		$current_level = $this->astronomia_user_model->get_level();
		...
		if($current_level==$level){
			...
			if(!file_exists('./contents/events/astronomia/'.$file))
			{
				header('HTTP/1.0 404 Not Found');
				redirect("oops404",'location');
				return;
				...
				header("Expires: Sat, 26 Jul 2016 05:00:00 GMT");
				header("Pragma: cache");
				echo file_get_contents('contents/events/astronomia/'.$file);
			}
		}else{
