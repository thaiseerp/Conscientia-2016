...
class Ion_auth
{
   ...
   * Custom Created Function
    */
	...
	public function register_email($email,$password,$group_ids = array()) //need to test email activation
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register_email($email,$password,$group_ids);
	 ...
	 * @author Thaiseer
	 **/
	public function register_user($user_id,$details = array())
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$id = $this->ion_auth_model->register_user($user_id,$details);
    ...
    public function get_signedup($id) {
        $signedup = $this->ion_auth_model->get_signed_up($id);
        if($signedup->is_signedup==1)
        return TRUE;
        else
        return FALSE;
    }
	 ...
	 * @return bool
	 **/
	public function is_evadmin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_evadmin');

		$evadmin_group = $this->config->item('evadmin_group', 'ion_auth');

		return $this->in_group($evadmin_group, $id);
	}
	 ...
	 * @return bool
	 **/
	public function is_wsadmin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_wsadmin');

		$wsadmin_group = $this->config->item('wsadmin_group', 'ion_auth');

		return $this->in_group($wsadmin_group, $id);
	}
	 ...
	 * @return bool
	 **/
	public function is_hsadmin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_hsadmin');

		$hsadmin_group = $this->config->item('hsadmin_group', 'ion_auth');

		return $this->in_group($hsadmin_group, $id);
	}
	 ...
	 * @return bool
	 **/
	public function is_banned($id=false)
	{
		$this->ion_auth_model->trigger_events('is_banned');

		$banned_group = $this->config->item('banned_group', 'ion_auth');

		return $this->in_group($banned_group, $id);
	}
