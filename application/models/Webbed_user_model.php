<?php
class Webbed_user_model extends CI_Model {
	...
	function register(){
		...
		if (is_array($sql)&&count($sql)==1){
			...
			return $this->db->insert('webbed_users',$data);
		}
	...
	function get_role($user_id=''){
		...
		$sql = "SELECT role FROM webbed_users WHERE user_id = ?"; 
	...
	function read_user($user_id=''){
		...
		$rank = $this->webbed_status_model->get_rank($user_id);

		$sql = "SELECT name,level, phone, college, email, role FROM webbed_users WHERE user_id = ?"; 
		$query = $this->db->query($sql, $user_id);
		if ($query->num_rows() > 0)
		{
		   $row = $query->row();
