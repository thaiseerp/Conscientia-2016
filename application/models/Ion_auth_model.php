...
/*		else
    ...
    public function get_signed_up($id) {
        $query = $this->db->select('is_signedup')
                      ->where('id', $id)
                      ->limit(1)
                      ->order_by('id', 'desc')
                      ->get($this->tables['users']);
	...
	public function register_user($user_id, $details = array())
	{
		...
		// filter out any data passed that doesnt have a matching column in the users table
		// and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['users'], $details), $data);
	 ...
	 * custom function
	 **/
	public function register_email($email,$password,$groups = array())
	{
		$this->trigger_events('pre_register');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, fb_uid, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, fb_uid, email, last_login')
