<?php
class Astronomia_levels_model extends CI_Model {
	...
	function get_level() {
		...
		$sql = 'SELECT level, title, difficulty, content, background, placeholder, cookie, javascript, html_comment, success_image, status FROM astronomia_levels WHERE level = ?'; 
	...
	function check_answer($answer){
		...
		$sql = "SELECT level,name FROM astronomia_users WHERE user_id = ?"; 
		$query = $this->db->query($sql, $user_id);
		if ($query->num_rows() > 0)
		{
		   $row = $query->row();
		...
		$this->db->insert('astronomia_log_answers',$log);

		//read the hash of correct answer
		$sql = "SELECT answer FROM astronomia_levels WHERE level = ?"; 
		$query = $this->db->query($sql, $current_level);
		...
		if($query->num_rows() > 0){
			...
			if(md5($answer) === $row->answer){
				...
				$this->db->update('astronomia_users'); 
