<?php
class Webbed_status_model extends CI_Model {
	...
	function get_leaderboard(){
		...
		$sql = "SELECT name, level, college, role FROM webbed_users ORDER BY level DESC, passtime ASC"; 
	...
	function get_rank($user_id=''){
		...
		$sql = "SELECT role FROM webbed_users WHERE user_id = $user_id"; 
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0)
		{
			$row = $query->row();
		...
		$sql = "SELECT user_id FROM webbed_users WHERE role=1 ORDER BY level DESC, passtime ASC"; 
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0)
		{
			foreach ($query->result() as $row)
